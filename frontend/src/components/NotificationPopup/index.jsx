...
const NotificationPopup = (props) => {
    ...
    const isOccurringTrip = (trip) => {
        ...
        if (!trip.endDate || trip.endDate === '') {
            return moment(today, 'YYYY-MM-DD').isSame(moment(trip.beginDate, 'YYYY-MM-DD'))
        } else {
            return !moment(today, 'YYYY-MM-DD').isBefore(moment(trip.beginDate, 'YYYY-MM-DD')) &&
    ...
    useEffect(() => {
        ...
        const getOccurringTrip = async () => {
            try {
                const response = await fetch(`${API_URL}/api/users/${user.username}/trips?target=me`);
                        ...
                        <PopupContent className={isActive ? 'active' : 'inactive'}>
                            ...
                            <TripInfo>
                                ...
                                <TripDate>
                                    ...
                                    <span>{occuringTrip.beginDate} {occuringTrip.endDate && occuringTrip.endDate !== "" && ` - ${occuringTrip.endDate}`}</span>
                                </TripDate>
                                <TripMates>
                                    <div className="icon_wrapper">
                                    ...
                                    <div className="trip_mates">
                                        {
                                            ...
                                            occuringTrip.tripMates.map(item => (
                                                <Link key={item} to={`/profile/${item}`} className="user_tag">
                                                    <span >{item}</span>
                                                </Link>
                                            ))
                                        }
                                ...
                                <TripTransportation>
                                    ...
                                    <span>{
                                        ...
                                        occuringTrip.transportation.slice(1).toLowerCase()}</span>
                                </TripTransportation>
                            </TripInfo>
                        </PopupContent>
                    </PopupContainer>
